...
class DashboardActivity : BaseActivity<ActivityDashboardBinding>() {
    ...
    override fun isInternetAvailable(status: Status) {
        when (status) {
            Status.Available -> Toast.makeText(this, Status.Available.name, Toast.LENGTH_SHORT)
                .show()
            Status.Unavailable -> Toast.makeText(this, Status.Unavailable.name, Toast.LENGTH_SHORT)
                .show()
        }
    }
